...
def get_local_idx_by_title(title: str, indices: pd.Series) -> int:
    ...
    if title not in indices:
        raise HTTPException(
            status_code=404, detail=f"Movie title '{title}' not found in local database"
        )
...
def tfidf_movie_recommendations(
    title: str, indices: pd.Series, tfidf_matrix: Any, df: pd.DataFrame, n: int = 10
